...
uint32_t classifyNode(struct hand_and_bid *node)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        temp_hand[i] = node->hand[i];
        if (node->hand[i] == 1)
        {
            jacks++;
            continue;
        }
        if (highest_card < node->hand[i])
        {
            highest_card = node->hand[i];
            highest_pos = i;
        }
    }
    ...
    for (int i = 0; i < 5; i++)
    {
        if (node->hand[i] == 1)
        {
            node->hand[i] = highest_card - 1;
            temp_hand[i] = highest_card;
        }
        else
        {
            for (int j = i + 1; j < 5; j++)
